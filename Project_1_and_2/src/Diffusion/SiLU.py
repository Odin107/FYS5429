...
with open('runtime_SiLu.txt', 'w') as file:
    file.write(f'Total time taken for training: {int(minutes)} minutes {seconds:.2f} seconds\n')

# Plot loss
plt.plot(epoch_list, loss_list)
plt.title('MSE for SiLU Activation Function')
plt.yscale('log')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.savefig(os.path.join(figures_dir, f'loss_Silu.png'))
plt.close()
...
plt.xlabel('x')
plt.ylabel('y')
plt.savefig(os.path.join(figures_dir, 'initial_condition_SiLU.png'))
plt.close()
...
for t in time_points:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, f'solution_SiLU_t_{t}.png'))
    plt.close
